...
with h1: st.markdown(f"## 📦 BRN WMS | Hoş geldin, {st.session_state.user.upper()}")
with h2:
...
if st.session_state.current_screen == "MAIN":
    ...
    with st.container(border=True):
        sa, sk, sm = st.columns(3)
        adr = sa.text_input("📍 Adres:").upper()
        kod = sk.text_input("📦 Kod:").upper()
        mik = sm.number_input("Miktar:", min_value=0.0)
        if st.button("➕ LİSTEYE EKLE", use_container_width=True):
            st.session_state['gecici_sayim_listesi'].append({"Adres": adr, "Kod": kod, "Miktar": mik})
            st.rerun()
    st.write(st.session_state['gecici_sayim_listesi'])

# --- ÜRÜN ADI (İSİM) GERİ GELDİ ---
elif st.session_state.current_screen == "SAYIM_FARK":
    if st.button("⬅️ ANA MENÜ"): set_screen("MAIN")
    st.title("⚖️ Envanter Fark Raporu")
    ...
    if not df_say.empty and not df_stk.empty:
        try:
            ...
            # 2. Stok (Sistem) verilerini topla - İSİM SÜTUNUNU BURADA KORUYORUZ
            t_g = df_stk.groupby(['Adres', 'Kod', 'İsim'])['Miktar'].sum().reset_index()
            t_g.columns = ['Adres', 'Kod', 'İsim', 'Sistem']
            
            # 3. İki tabloyu birleştir
            rapor = pd.merge(s_g, t_g, on=['Adres', 'Kod'], how='left').fillna({'İsim': 'Tanımsız Ürün', 'Sistem': 0})
            rapor['FARK'] = rapor['Sayilan'] - rapor['Sistem']
            
            # Filtreler
            f1, f2 = st.columns(2)
            fa = f1.text_input("📍 Adres Filtre:").upper()
            fk = f2.text_input("📦 Kod Filtre:").upper()
            if fa: rapor = rapor[rapor['Adres'].str.contains(fa)]
            if fk: rapor = rapor[rapor['Kod'].str.contains(fk)]
            ...
            st.error(f"Rapor hatası: {e}. Lütfen Stok sayfasında 'Adres', 'Kod', 'İsim', 'Miktar' başlıklarını kontrol edin.")

elif st.session_state.current_screen == "STOK":
    if st.button("⬅️ ANA MENÜ"): set_screen("MAIN")
    st.title("📊 Stok Hareket Girişi")
    st.info("Bu ekran veri tabanına yazma aşamasındadır.")
